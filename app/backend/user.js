					...
					if(req.body.password==undefined || req.body.password==null || req.body.password.length < 5){ 
						...
						if(ce!=0){
							...
							if(this.config.smtp.enabled===true){
								...
								memo.html = this.render.processTemplateByPath(this.dir + this.config.properties.views + "user/mail.activate.html", memo);
			...
			case "post":
				...
				if(rows.length!=1){
					...
					if(this.helper.toHash(req.body.password+req.body.email,rows[0].hash) != rows[0].password){
						...
						}else if(this.config.on && this.config.on.login){
							res.redirect(this.config.on.login);
						}else{
	...
	try{
		...
		if(req.query.xhr){
			res.send({data: true});
				...
				res.redirect(this.config.on.logout);
		}else{
		...
		switch(req.method.toLowerCase()){
			...
			case "post":
				...
				if(user.length!=1){
					...
					memo.html = this.render.processTemplateByPath(this.dir + this.config.properties.views + "user/mail.recovery.html", memo);
					await this.mailing.send(memo);
					if(req.body.xhr){
						res.send({data: true});
					}else{
